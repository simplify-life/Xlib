...
namespace xlib {
    ...
    namespace crypto{
        ...
        void R2(uint32 block[BLOCK_INTS], const uint32 v, uint32 &w, const uint32 x, const uint32 y, uint32 &z, const uint64 i)
        {
            block[i] = blk(block, i);
        ...
        void buffer_to_block(const std::string &buffer, uint32 block[BLOCK_INTS])
        {
            /* Convert the std::string (byte buffer) to a uint32_t array (MSB) */
            for (uint64 i = 0; i < BLOCK_INTS; i++)
            {
        ...
        std::string SHA1::final()
        {
            ...
            {
                transform(digest, block, transforms);
                for (uint64 i = 0; i < BLOCK_INTS - 2; i++)
                {
                    block[i] = 0;
                }
            }
